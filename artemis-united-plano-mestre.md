...
> **Nota de implementação — RESOLVIDA (2026-07-24)**: as decisões #27 e #34 descreviam o DRE como wizard de 6 blocos com salvamento parcial, mas a implementação real (`DreForm.tsx`) era um formulário único de 6 campos numa tela só, achado na Fase 1/5 do reforço de QA. Fundador revisou e confirmou o wizard (decisão #48) — reconstruído: 6 telas de bloco único + tela de resumo antes de calcular, "voltar" preserva os valores já digitados (salvamento parcial em memória entre blocos, não persistido no servidor por bloco), "cancelar" sai do wizard sem submeter. Eventos `dre_block_completed`/`dre_block_abandoned` (seção 7) agora instrumentados. Contrato do backend (`financial_statements`) não mudou — continua um único POST no bloco de resumo, e continua nunca sobrescrevendo (linha nova por submissão).
...
| 47 | Reforço de QA (2026-07-24) identificou rate limiting de login e redação de PII em logs estruturados (apps/api/bob-engine) como as duas mitigações do modelo de ameaças com maior lacuna risco/esforço | Segurança | Priorização de implementação pendente de confirmação do fundador |
| 48 | Decisões #27/#34 (DRE como wizard de 6 blocos + resumo) revisadas e confirmadas — reconstruído em código, não revertido pra formulário único | Design UX/UI | Rationale original (ICP com fluência/tempo fragmentados, mobile-first) segue válido; nenhum dado real de uso surgiu que o invalidasse. Formulário único era corte de escopo da implementação inicial, não uma decisão de produto considerada |
