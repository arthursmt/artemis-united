...
**Nota técnica — RESOLVIDA (confirmado na Fase 4 do reforço de QA, 2026-07-24)**: `assessment_outcomes` (schema `bob`, decisão #15) já tem os quatro campos — `effective_interest_rate`, `term_months`, `collateral_description`, `owner_equity_contributed` — implementados desde a migração `0001_abnormal_lenny_balinger.sql` (ver `services/bob-engine/src/db/schema.ts`). Nenhuma migração nova precisa ser gerada; o gap descrito abaixo já estava fechado antes desta checagem, só não tinha sido confirmado por escrito neste documento.
